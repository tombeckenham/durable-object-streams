...
- **The one transport-level flake (SSE CRLF-injection tests against the live URL)**: the suite's `fetchSSE` helper stops reading as soon as its content marker appears in the buffer followed by any event boundary — and in these tests the marker text is embedded *inside the data payload*. Measured empirically: Cloudflare's edge splits the origin's single ~216-byte SSE write into two chunks on every response (varying split points), so whether the reader sees the complete event pair depends on whether undici happens to coalesce the chunks before its next `read()` — a client-side race no origin behavior can influence (the origin already batches each data+control pair into one write precisely for this reason, which is sufficient on localhost). Manual verification against the live URL shows correctly framed, CRLF-safe events delivered immediately. `--retry=2` absorbs this.

...
- **Live (deployed) runs use `--testTimeout=30000`**: vitest's 5s default per-test budget assumes a local server; the property-based groups issue 100+ sequential HTTP round-trips per test, which is arithmetically impossible in 5s over real network RTT (11 tests time out even from a GitHub runner ~25ms from Cloudflare's edge, with zero assertion failures). The extended timeout changes no test logic or assertions.
